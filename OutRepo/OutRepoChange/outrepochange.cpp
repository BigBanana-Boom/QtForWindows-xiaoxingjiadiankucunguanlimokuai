...
OutRepoChange::OutRepoChange(QWidget *parent,
                             QSqlDatabase *db,
                             QSqlQuery *query)
    : QWidget(parent),
      ...
      qlabel01(new QLabel("待修改出库编号：", this)),
      qlabel02(new QLabel("待修改出库时间：", this)),
      qlabel03(new QLabel("待修改出库类别：", this)),
      qlabel04(new QLabel("待修改出库名称：", this)),
      qlabel05(new QLabel("待修改出库位置：", this)),
      qlabel06(new QLabel("待修改出库数量：", this)),
      qlabel07(new QLabel("修改后出库数量：", this)),
      ...
      changedproductnumber(new int)
{
    ...
    sqlgroup->append("UPDATE 出库表 SET 出库数量 = ? WHERE 出库编号 = ?");
    /* 数据库操作语句，不需要排序 */
    ...
    if(*currentID == 0) {
        ...
        *changedproductnumber = 0;
        ...
        if(query->next()) {
            ...
            qspinbox01->setValue(query->value(0).toInt()+*currentproductnumber);
        } else {
            qspinbox01->setMinimum(1);
    ...
    connect(searchbutton, &QPushButton::clicked,
            this, &OutRepoChange::onSearchButtonClicked);
    connect(qcombobox01, &QComboBox::currentTextChanged,
            this, &OutRepoChange::onChangedIDChanged);
    using SpinBoxSignal = void (QSpinBox::*)(int);
    SpinBoxSignal spinBoxSignal = &QSpinBox::valueChanged;
    connect(qspinbox01, spinBoxSignal, this, &OutRepoChange::onChangedValueChanged);
    connect(yeschange, &QPushButton::clicked, this,
            &OutRepoChange::showMessage);
    connect(yeschangeandsync, &QPushButton::clicked, this,
            &OutRepoChange::showMessage2);
    // 信号与槽函数*************************************************************************
}
OutRepoChange::~OutRepoChange() {
    // 删除new对象*************************************************************************
    delete qfont01;
    delete qfont02;
    delete qfont03;
...
void OutRepoChange::onChangedIDChanged(const QString &text) {
    // 记录更新*****************************************************************************
...
void OutRepoChange::onChangedValueChanged(int value) {
    // 记录更新*****************************************************************************
    *changedproductnumber = value;
    // 记录更新*****************************************************************************
}
void OutRepoChange::ProcessTable() {
    // 清理以防止内存泄漏******************************************************************
...
void OutRepoChange::showMessage()
{
    if(*currentID == 0) {
        // 新对象****************************************************************************
        simpledialog = new SimpleDialog(false, true, true, this);
        simpledialog->setDialogContent("出库表中暂无记录，无法修改");
        // 新对象****************************************************************************
        // 嗯，坏米饭************************************************************************
        // 调整位置**************************************************************************
        ...
        // 新对象****************************************************************************
        simpledialog = new SimpleDialog(false, true, true, this);
        simpledialog->setDialogContent("出库表无变化，修改无效");
        // 新对象****************************************************************************
        // 嗯，坏米饭************************************************************************
        // 调整位置**************************************************************************
        QRect parentGeometry = this->window()->window()->geometry();
        ...
        // 新对象****************************************************************************
        outrepochangedialog = new OutRepoChangeDialog(false, true, true, this);
        outrepochangedialog->setOutRepoChangeID(QString::number(*currentID));
        outrepochangedialog->setOutRepoChangeDateTime(
                    currentdatetime->toString("yyyy-MM-dd hh:mm:ss"));
        outrepochangedialog->setOutRepoChangeCategory(*currentproductcategory);
        outrepochangedialog->setOutRepoChangeName(*currentproductname);
        outrepochangedialog->setOutRepoChangeRepo(*currentrepo);
        outrepochangedialog->setOutRepoOriginalNumber(
                    QString::number(*currentproductnumber));
        outrepochangedialog->setOutRepoChangeNumber(
                    QString::number(*changedproductnumber));
        // 新对象****************************************************************************
        // 新对象的信号与槽*****************************************************************
        connect(outrepochangedialog, &OutRepoChangeDialog::accepted, this, [this]() {
        ...
        // 调整位置**************************************************************************
        QRect parentGeometry = this->window()->window()->geometry();
        int dialogWidth = outrepochangedialog->width();
        int dialogHeight = outrepochangedialog->height();
        int x = (parentGeometry.width() - dialogWidth) / 2 + parentGeometry.x();
        int y = (parentGeometry.height() - dialogHeight) / 2 + parentGeometry.y();
        // 调整位置**************************************************************************
        // 移动对话框***********************************************************************
        outrepochangedialog->move(x, y);
        // 移动对话框***********************************************************************
        // 嗯，坏米饭************************************************************************
        outrepochangedialog->exec();
        delete outrepochangedialog;
    }
}
void OutRepoChange::SubmitChangeOperation() {
    // 更新数据表***************************************************************************
    // 更新出库表***************************************************************************
    ...
    // 放出信号******************************************************************************
    // 更新自己**************************************************************************
    RefreshOutRepoChangeSlot();
    // 更新自己**************************************************************************
}
void OutRepoChange::showMessage2() {
    ...
    if(*currentID == 0) {
        ...
        // 新对象****************************************************************************
        outrepochangedialog = new OutRepoChangeDialog(false, true, true, this);
        outrepochangedialog->setOutRepoChangeTitle("修 改 出 库 并 同 步");
        outrepochangedialog->setOutRepoChangeID(QString::number(*currentID));
        outrepochangedialog->setOutRepoChangeDateTime(
                    currentdatetime->toString("yyyy-MM-dd hh:mm:ss"));
        outrepochangedialog->setOutRepoChangeCategory(*currentproductcategory);
...
void OutRepoChange::SubmitChangeOperation2() {
    // 更新数据表***************************************************************************
    // 更新出库表***************************************************************************
    query->prepare(sqlgroup->at(7));
    query->addBindValue(*changedproductnumber);
    ...
    query->exec();
    if(query->next()) {
        int leftnumber = query->value(0).toInt();
        query->prepare(sqlgroup->at(8));
        query->addBindValue(leftnumber + *currentproductnumber);
        query->addBindValue(*currentrepo);
        query->addBindValue(*currentproductcategory);
        query->addBindValue(*currentproductname);
        query->exec();
    ...
    query->addBindValue(*currentproductname);
    query->exec();
    if(query->next() && query->value(0).toInt() == *changedproductnumber) {
        ...
        query->exec();
        RefreshRepoTableID refreshid(db, query);
    } else if(query->value(0).toInt() != *changedproductnumber) {
...
void OutRepoChange::RefreshOutRepoChangeSlot() {
    // 关信号****************************************************************************
...
void OutRepoChange::onSearchButtonClicked() {
    // 新对象****************************************************************************
    returnid = new int;
    *returnid = *currentID;
    searchdialog = new SearchDialog(true, true, true, this, db, query, returnid);
    searchdialog->setDialogTitle("修 改 出 库 · 查 找");
