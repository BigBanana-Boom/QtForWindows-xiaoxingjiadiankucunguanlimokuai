#include "outrepochangedialog.h"

OutRepoChangeDialog::OutRepoChangeDialog(bool resizeEnable,
                                         bool shadowBorder,
                                         bool winNativeEvent,
                                         QWidget *parent)
    ...
    rowSixContent(new QLabel(this)),
    rowSixHLayout(new QHBoxLayout()),
    rowSevenLabel(new QLabel(this)),
    rowSevenContent(new QLabel(this)),
    rowSevenHLayout(new QHBoxLayout()),
    rowEightLabel(new QLabel(this)),
    ...
    CancelButton(new DialogGradientButton(this))
{
    ...
    // 标题******************************************************************************
    contentWidgetTitle->setText("修 改 出 库");
    ...
    // 第五行****************************************************************************
    // 第六行****************************************************************************
    rowSixLabel->setText("待修改出库数量：");
    rowSixLabel->setFont(*qfont03);
    // 让外类通过公有方法设置rowSixContent的内容************************************
    rowSixContent->setFont(*qfont04);
    rowSixHLayout->addWidget(rowSixLabel);
    rowSixHLayout->addWidget(rowSixContent, 1);
    contentWidgetVLayout->addLayout(rowSixHLayout);
    // 第六行****************************************************************************
    // 第七行****************************************************************************
    rowSevenLabel->setText("修改后出库数量：");
    rowSevenLabel->setFont(*qfont03);
    // 让外类通过公有方法设置rowSevenContent的内容*********************************
    rowSevenContent->setFont(*qfont04);
    rowSevenHLayout->addWidget(rowSevenLabel);
    rowSevenHLayout->addWidget(rowSevenContent, 1);
    contentWidgetVLayout->addLayout(rowSevenHLayout);
    // 第七行****************************************************************************
    // 第八行****************************************************************************
    rowEightLabel->setText("确定修改该记录嘛？");
    rowEightLabel->setFont(*qfont03);
    contentWidgetVLayout->addWidget(rowEightLabel);
    // 第八行****************************************************************************
    // 跟该Pro的其它保持一致***********************************************************
    contentWidgetVLayout->setSpacing(8);
    contentWidgetVLayout->setContentsMargins(20, 8, 20, 8);
    // 跟该Pro的其它保持一致***********************************************************
    contentWidget->setFixedHeight(415);
    // 内容器件**************************************************************************
    ...
    connect(closebutton, &QPushButton::clicked,
            this, &OutRepoChangeDialog::onCloseButtonClicked);
...
OutRepoChangeDialog::~OutRepoChangeDialog()
{
    // 删除new对象**********************************************************************
    delete qfont01;
    delete qfont02;
...
void OutRepoChangeDialog::onCloseButtonClicked()
{
    // 关闭窗口***************************************************************************
    close();
    // 关闭窗口***************************************************************************
}
void OutRepoChangeDialog::setOutRepoChangeTitle(QString title) {
    contentWidgetTitle->setText(title);
}
void OutRepoChangeDialog::setOutRepoChangeID(QString id) {
    rowOneContent->setText(id);
}
void OutRepoChangeDialog::setOutRepoChangeDateTime(QString datetime) {
    rowTwoContent->setText(datetime);
}
void OutRepoChangeDialog::setOutRepoChangeCategory(QString category) {
    rowThreeContent->setText(category);
}
void OutRepoChangeDialog::setOutRepoChangeName(QString name) {
    rowFourContent->setText(name);
}
void OutRepoChangeDialog::setOutRepoChangeRepo(QString repo) {
    rowFiveContent->setText(repo);
}
void OutRepoChangeDialog::setOutRepoOriginalNumber(QString number) {
    rowSixContent->setText(number);
}
void OutRepoChangeDialog::setOutRepoChangeNumber(QString number) {
    rowSevenContent->setText(number);
}
/*
 * TotalVLayout->setContentsMargins(0, 0, 0, 0);
