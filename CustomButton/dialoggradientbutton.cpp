#include "dialoggradientbutton.h"

DialogGradientButton::DialogGradientButton(QWidget* parent)
    : QPushButton(parent),
      ...
      currentStep(0)
{
    ...
    setStyleSheet(QString("border: solid; background-color: %1").arg(currentColor.name()));
    // 默认颜色**************************************************************************
    // 初始化定时器**********************************************************************
    ...
    connect(gradientTimer, &QTimer::timeout, this,
            &DialogGradientButton::updateGradient);
    // 槽函数*****************************************************************************
}
void DialogGradientButton::setNormalColor(const QColor& color) {
    normalColor = color;
    currentColor = color;
    /* 恩，嗯，坏米饭*/
    gradientTimer->stop();
...
void DialogGradientButton::setHoverColor(const QColor& color) {
    hoverColor = color;
}
void DialogGradientButton::enterEvent(QEvent* event) {
    // 设置目标颜色为悬停颜色***********************************************************
...
void DialogGradientButton::leaveEvent(QEvent* event) {
    // 设置目标颜色为正常颜色************************************************************
...
void DialogGradientButton::updateGradient() {
    // 停止*******************************************************************************
