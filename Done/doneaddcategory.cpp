#include "doneaddcategory.h"

DoneAddCategory::DoneAddCategory(QWidget *parent,
                                 QSqlDatabase *db,
                                 QSqlQuery *query)
    : QDialog(parent),
      ...
      sqlgroup(new QStringList())
{
    ...
    sqlgroup->append("INSERT INTO 附表2进货产品表 (产品类别, 产品名称) VALUES (?, '暂无')");
    // 数据库*******************************************************************************
    ...
    connect(okButton, &QPushButton::clicked, this, &DoneAddCategory::SaveCategory);
}
DoneAddCategory::~DoneAddCategory() {
    delete sqlgroup;
}
void DoneAddCategory::SaveCategory() {
    if(lineEdit->text() == "") {
