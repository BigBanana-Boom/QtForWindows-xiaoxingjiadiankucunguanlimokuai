#include "doneaddname.h"

DoneAddName::DoneAddName(QWidget *parent,
                         QSqlDatabase *db,
                         QSqlQuery *query,
                         QString *category)
    : QDialog(parent),
      label0(new QLabel("当前已定类别：", this)),
      ...
      category(category)
{
    ...
    connect(okButton, &QPushButton::clicked, this, &DoneAddName::SaveName);
}
DoneAddName::~DoneAddName() {
    delete sqlgroup;
}
void DoneAddName::SaveName() {
    if(lineEdit->text() == "") {
